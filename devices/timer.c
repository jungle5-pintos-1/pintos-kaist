...
// loops_per_tick 값을 조정하여, 시스템의 성능에 따라 적절한 지연을 생성할 수
// 있는 루프의 수를 결정 간단한 반복 루프를 사용하여 시간 지연을 보다 정밀하게
// 조정
...
void timer_sleep(int64_t ticks) {
  ...
  if (timer_elapsed(start) < ticks)
    thread_sleep(start + ticks);  // 지정된 ticks 시간이 경과하지 않았으면
                                  // thread_sleep을 호출하여 스레드 대기
}
...
// 타이머 인터럽트가 발생할 때마다 호출되어 'ticks' 변수를 증가시키고, 스레드
// 관리자를 통해 스레드의 타이밍을 조정
