 ...
 * 이 함수는 FILE_NAME으로 지정된 프로그램을 로드하여 실행하는 새로운 스레드를
 * 생성합니다 */
...
// 첫 번째 사용자 프로세스를 시작하는 스레드 함수, 주어진 파일 이름으로 프로그램
// 로드, 실행
static void initd(void *f_name) {  // 실행할 파일의 이름을 나타내는 포인터
  ...
  NOT_REACHED();  // 여기에 도착하면 프로그래밍 오류(process_exec에서 프로세스
                  // 시작되어 initd는 더이상 실행되면 안된다)
}
...
int process_exec(void *f_name) {
  ...
  _if.eflags = FLAG_IF | FLAG_MBS;  // 인터럽트 활성화 플래그, 필수 플래그
                                    // (사용자 모드에서 안전한 실행 보장)

  /* We first kill the current context */
  // 현재 컨텍스트 정리
  process_cleanup();  // 새 프로그램을 로드하기 전에 현재 프로세스의 메모리 할당
                      // 등을 해제하여 상태를 초기화

  /* --- argument parsing --- */
  char *parse[64];  // command line의 길이가 128로 제한 -> 인자와 공백을 합치면
                    // 2바이트, 최대 64개의 인자 구분 가능
  char *save_ptr;
  int count = 0;
  ...
  // 이진 파일 로드
  // printf("---%s---\n", file_name);
  success = load(file_name, &_if);  // 로드 함수를 호출하여 파일 이름에 해당하는
                                    // 프로그램을 메모리에 로드
  ...
  // hex_dump(_if.rsp, _if.rsp, USER_STACK - (uint64_t)_if.rsp, true); // user
  // stack을 16진수로 프린트
...
void argument_stack(char **parse, int count, void **rsp) {
  ...
  for (int i = count - 1; i >= 0; i--) {
    for (int j = strlen(parse[i]); j >= 0;
         j--)  // '\0'이 있으므로 strlen(parse[i]) 부터 시작
    {
      (*rsp)--;                      // 스택 주소 감소 (1바이트)
      **(char **)rsp = parse[i][j];  // 주소에 문자 저장
    }
    parse[i] = *(
        char **)rsp;  // parse[i]에 현재 rsp의 값(각 인자의 시작 주소) 저장해둠
  }
  ...
  **(char ***)rsp = 0;  // char* 타입의 0 추가
  ...
  // 리턴 주소 푸시
  (*rsp) -= 8;
  **(void ***)rsp = 0;  // void* 타입 0 추가 (fake return address, 프로세스
                        // 생성이라 반환 주소가 없다)
}
...
void process_exit(void) {
  ...
  for (int fd = 2; fd < FDT_COUNT_LIMIT; fd++) {
    if (curr->file_descriptor_table[fd] != NULL) {
      file_close(curr->file_descriptor_table[fd]);  // file_allow_write 도 같이 해주고 있음
 ...
 * 해당 실행 파일의 엔트리 포인트(프로그램이 실행될 때 처음으로 실행을 시작하는
 * 코드의 주소)와 초기 스택 포인터를 intr_frame 구조체에 저장하는 과정 수행 */
static bool load(const char *file_name, struct intr_frame *if_) {
  ...
  if (t->pml4 == NULL) goto done;  // 실패하면 done 으로
  ...
  if (file_read(file, &ehdr, sizeof ehdr) !=
          sizeof ehdr  // file_read 통해 ELF 헤더를 읽음
  ...
  for (i = 0; i < ehdr.e_phnum; i++) {  // 헤더를 순차적으로 읽는다
    struct Phdr phdr;
    ...
    switch (phdr.p_type) {
      ...
      case PT_SHLIB:
        goto done;
      case PT_LOAD:  // PT_LOAD 타입 세그먼트는 load_segment() 함수를 통해
                     // 메모리에 로드
