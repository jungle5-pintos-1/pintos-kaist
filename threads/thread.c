...
bool less_wake_up_tick(const struct list_elem *a, const struct list_elem *b,
                       void *aux UNUSED);
...
void thread_init(void) {
  ...
  initial_thread = running_thread(); // 현재 실행 중인 스레드 반환
  init_thread(initial_thread, "main", PRI_DEFAULT); // 현재 실행 중인 스레드(main 스레드)를 구조체로 저장
...
void thread_start(void) {
  ...
  thread_create("idle", PRI_MIN, idle, &idle_started);  // 유휴스레드 생성
  load_avg = LOAD_AVG_DEFAULT;
  /* Start preemptive thread scheduling. */
  intr_enable(); // 인터럽트 활성화 -> 스케줄러 활성화
...
tid_t thread_create(const char *name, int priority, thread_func *function, void *aux) {
  ...
  /* Initialize thread. */
  init_thread(t, name, priority); // 스레드, 이름, 우선순위를 구조체에 저장
...
void thread_yield(void) {
  ...
  old_level = intr_disable();  // 현재 인터럽트 레벨을 비활성화 -> 다른
                               // 인터럽트의 방해 피하기 위해
  if (curr != idle_thread)  // 현재 스레드가 idle이 아닐 경우
    // list_push_back(&ready_list, &curr->elem); // 준비리스트 뒤쪽에 추가
    list_insert_ordered(&ready_list, &curr->elem, cmp_priority,
                        NULL);  // 우선순위 확인하여 준비리스트에 삽입
  do_schedule(
      THREAD_READY);  // 스레드의 상태를 READY로 설정, 스케줄러 호출, 새로운
                      // 스레드를 선택하고 실행 (스레드가 자발적으로 CPU양보)
  intr_set_level(old_level);  // 인터럽트 레벨 복원 -> 활성화
}

// 두 스레드를 비교, list_elem 포인터를 실제 구조체로 변환 후 각 스레드의
// wakeup_tick을 비교 a가 작으면 true
bool less_wake_up_tick(const struct list_elem *a, const struct list_elem *b,
                       void *aux UNUSED) {
...
void thread_sleep(int64_t ticks) {
  ...
  list_insert_ordered(&sleep_list, &curr->elem, less_wake_up_tick,
                      NULL);  // 슬립 스레드에 순서대로 삽입
...
void thread_wake_up(int64_t ticks) {
  ...
  old_level = intr_disable();
  struct list_elem *e =
      list_begin(&sleep_list);  // 슬립리스트의 시작 엘리멘트 포인터
  while (e != list_end(&sleep_list))  // 시작부터 끝까지 순회
  {
    struct thread *t =
        list_entry(e, struct thread, elem);  // e가 가리키는 thread 구조체
...
void thread_set_priority(int new_priority) {
  ...
  current_thread->init_priority =
      new_priority;  // 새로 들어온 우선순위를 현재 우선순위로
  update_priority_before_donations();  // thread의 우선순위가 변경되면 donaitons
                                       // 정보를 갱신
  preempt_priority();
}
...
void mlfqs_calculate_priority(struct thread *t) {
  ...
  // fp 연산 함수를 사용하여 계산 결과의 소수 부분은 버리고 정수의 priority로
  // 설정
  t->priority =
...
void mlfqs_recalculate_recent_cpu(void) {
  ...
  */
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
    ...
    여기에는 스레드의 상태, 우선 순위, 스케줄링 관련 정보 등이 포함될 수
    있습니다.
    */
